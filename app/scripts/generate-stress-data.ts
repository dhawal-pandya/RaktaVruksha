  ...
  for (const parentUnion of currentUnions) {
    ...
    for (let k = 0; k < nKids && people.length < TARGET; k++) {
      ...
      if (gen < MAX_GEN && chance(0.62)) {
        ...
        const spouseFamily = pick(
          familyIds.filter((f) => f !== parentUnion.familyId),
        );
        ...
        const intoFamily =
          child.gender === "male"
            ? child.birthFamilyId
            : spouseFamily === child.birthFamilyId
              ? spouseFamily
              : spouse.birthFamilyId && chance(0.15)
                ? spouse.birthFamilyId
                : child.birthFamilyId;
        const status: UnionStatus = chance(0.05)
          ? "divorced"
          : chance(0.04)
            ? "partners"
            : "married";
        ...
        if (status === "divorced" && chance(0.7)) {
          ...
          const spouse2 = makePerson(
            child.gender === "male" ? "female" : "male",
            f2,
          );
          const u2 = makeUnion(
            [child.id, spouse2.id],
            child.gender === "male" ? child.birthFamilyId : f2,
            "married",
          );
          u2.order = 2;
          nextUnions.push(u2);
...
console.log(
  `stress data: ${people.length} people, ${unions.length} unions, ${familyIds.length} families`,
);
console.log(
  `  divorced: ${unions.filter((u) => u.status === "divorced").length}, partners: ${unions.filter((u) => u.status === "partners").length}, single-parent: ${unions.filter((u) => u.partners.length === 1).length}, adoptions: ${unions.reduce((n, u) => n + (u.adoptedChildren?.length ?? 0), 0)}`,
);
if (warnings.length)
  console.log(`  warnings: ${warnings.length} (first: ${warnings[0]})`);
if (errors.length) {
