...
for (const fid of Object.keys(data.families))
  if (!uglyFamily(fid)) taken.add(fid);

const personMap = new Map<string, string>();
for (const p of data.people) {
  personMap.set(
    p.id,
    uglyPerson(p.id) ? uniq(clean(p.firstName) || "person") : p.id,
  );
}
...
const mapFam = (fid: string | null) =>
  fid == null ? null : (familyMap.get(fid) ?? fid);
const mapPerson = (pid: string) => personMap.get(pid) ?? pid;

const next: FamilyDataV2 = {
  meta: data.meta,
  families: Object.fromEntries(
    Object.entries(data.families).map(([fid, fam]) => [
      familyMap.get(fid) ?? fid,
      fam,
    ]),
  ),
  people: data.people.map((p) => ({
    ...p,
    id: mapPerson(p.id),
    birthFamilyId: mapFam(p.birthFamilyId),
  })),
...
console.log(
  `unions renamed: ${[...unionMap].filter(([a, b]) => a !== b).length}`,
);
