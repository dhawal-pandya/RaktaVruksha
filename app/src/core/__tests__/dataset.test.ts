...
describe('validateData', () => {
  ...
  it('rejects a person as biological child of two unions', () => {
    ...
    expect(errors.some(e => e.includes('biological child of two unions'))).toBe(true);
  });
...
describe('buildDataset derived relations', () => {
  ...
  it('derives spouses with union status', () => {
    ...
    expect(dadSpouses).toContainEqual({ id: 'Ex', unionId: 'u_dad_ex', status: 'divorced' });
    expect(dadSpouses).toContainEqual({ id: 'Mom', unionId: 'u_dad_mom', status: 'married' });
  });
  ...
  it('handles the unknown-partner case as a 1-partner union', () => {
    expect(ds.parentsOf.get('OutKid')).toEqual([{ id: 'SoloMum', tag: 'biological' }]);
  });
  ...
  it('derives family history: birth, then married-into, in union order', () => {
    ...
    expect(ds.familiesOf.get('Ex')).toEqual([
      ...
      { familyId: 'famA', kind: 'married-into', status: 'divorced', unionId: 'u_dad_ex' },
    ]);
    // Mom: born famB, married into famA
    expect(ds.familiesOf.get('Mom')).toEqual([
      { familyId: 'famB', kind: 'birth' },
      { familyId: 'famA', kind: 'married-into', status: 'married', unionId: 'u_dad_mom' },
    ]);
